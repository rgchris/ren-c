...
***********************************************************************/
{
	...
	VAL_SET(&val, 123);
	if (VAL_TYPE(&val) != 123) CRASH_V(RP_REBVAL_ALIGNMENT);
	...
	if (sizeof(void *) == 8) {
		if (sizeof(REBVAL) != 32) CRASH_V(RP_REBVAL_ALIGNMENT);
		if (sizeof(REBGOB) != 84) CRASH_V(RP_BAD_SIZE);
	} else {
		if (sizeof(REBVAL) != 16) CRASH_V(RP_REBVAL_ALIGNMENT);
		if (sizeof(REBGOB) != 64) CRASH_V(RP_BAD_SIZE);
	}
	if (sizeof(REBDAT) != 4) CRASH_V(RP_BAD_SIZE);
}
...
***********************************************************************/
{
	...
	if (VAL_TAIL(&Boot_Block->types) != REB_MAX)
		CRASH_V(RP_BAD_BOOT_TYPE_BLOCK);

	if (VAL_WORD_SYM(VAL_BLK(&Boot_Block->types)) != SYM_END_TYPE)
		CRASH_V(RP_BAD_END_TYPE_WORD);
	...
	if (0 != strcmp("end!", AS_CHARS(Get_Sym_Name(SYM_END_TYPE))))
		CRASH_V(RP_BAD_END_CANON_WORD);

	if (0 != strcmp("true", AS_CHARS(Get_Sym_Name(SYM_TRUE))))
		CRASH_V(RP_BAD_TRUE_CANON_WORD);

	if (0 != strcmp("line", BOOT_STR(RS_SCAN, 1)))
		CRASH_V(RP_BAD_BOOT_STRING);
}
...
***********************************************************************/
{
	...
	for (word = VAL_BLK(&Boot_Block->ops); NOT_END(word); word+=2) {
		...
		if (!func) CRASH_V(RP_EARLY_ERROR);
		*val = *func;
		VAL_SET(val, REB_OP);
...
static void Set_Option_String(REBCHR *str, REBCNT field)
{
	...
	if (str) {
		...
		Set_String(val, Copy_OS_Str(str, LEN_OS_STR(str)));
	}
}
...
***********************************************************************/
{
	...
	if (NZ(data = OS_GET_LOCALE(0))) {
		...
		Set_String(val, Copy_OS_Str(data, LEN_OS_STR(data)));
	}
...
***********************************************************************/
{
	...
	PG_Mem_Usage = 0;
	PG_Mem_Limit = 0;
	PG_Reb_Stats = ALLOC_ZEROFILL(REB_STATS);
	Reb_Opts = ALLOC_ZEROFILL(REB_OPTS);
	...
	//Debug_Str(AS_CBYTES(BOOT_STR(RS_INFO,0))); // Booting...
